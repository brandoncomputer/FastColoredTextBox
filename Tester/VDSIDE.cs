...
namespace Tester
{
    public partial class VDSIDE : Form
    {
        ...
        {
            if (tsFiles.SelectedTab.Text != null)
                Save(tsFiles.SelectedTab);
        }

        private void saveAsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (tsFiles.TabPages != null)
            {
                string oldFile = tsFiles.SelectedTab.Tag as string;
                tsFiles.SelectedTab.Tag = null;
                if (!Save(tsFiles.SelectedTab))
                if(oldFile!=null)
                {
                        tsFiles.SelectedTab.Tag = oldFile;
                        tsFiles.SelectedTab.Text = Path.GetFileName(oldFile);
                }
            }
        }
        ...
        {
            get {
                if (tsFiles.SelectedTab == null)
                    return null;
                return (tsFiles.SelectedTab.Controls[0] as FastColoredTextBox);
            }

            set
            {
           //     tsFiles.SelectedTab = (value.Parent as FATabStripItem);
                value.Focus();
            }
        }
        ...
        private void tmUpdateInterface_Tick(object sender, EventArgs e)
        {
            try
            {
                if(CurrentTB != null && tsFiles.TabPages.Count > 0)
                {
                    var tb = CurrentTB;
        ...
        private void printToolStripButton_Click(object sender, EventArgs e)
        {
            if(CurrentTB!=null)
            {
                ...
                settings.Title =tsFiles.SelectedTab.Text;
        ...
        private bool NavigateBackward()
        {
            ...
            for (int iTab = 0; iTab < tsFiles.TabPages.Count; iTab++)
            {
                var t = (tsFiles.TabPages[iTab].Controls[0] as FastColoredTextBox);
            ...
            if (iLine >= 0)
            {
           //     tsFiles.SelectedTab = (tb.Parent as FATabStripItem);
                tb.Navigate(iLine);
